# file_name: bodyweight_squats_detector.py

import math
from app.engines.exercise.base_exercise import BaseExercise


class BodyweightSquatsDetector(BaseExercise):
    UP_THRESHOLD = 160
    DOWN_THRESHOLD = 90
    MIN_VISIBILITY = 0.7
    KNEE_VALGUS_TOLERANCE = 0.05  # Form check for knees caving in

    LEFT_HIP = 23
    RIGHT_HIP = 24
    LEFT_KNEE = 25
    RIGHT_KNEE = 26
    LEFT_ANKLE = 27
    RIGHT_ANKLE = 28
    LEFT_SHOULDER = 11
    RIGHT_SHOULDER = 12

    def __init__(self):
        super().__init__()

    def reset(self) -> None:
        self.reps = 0
        self.stage = None

    def process(self, landmarks) -> dict:
        key_landmarks_visible = (
            landmarks[self.LEFT_HIP].visibility > self.MIN_VISIBILITY and
            landmarks[self.RIGHT_HIP].visibility > self.MIN_VISIBILITY and
            landmarks[self.LEFT_KNEE].visibility > self.MIN_VISIBILITY and
            landmarks[self.RIGHT_KNEE].visibility > self.MIN_VISIBILITY and
            landmarks[self.LEFT_ANKLE].visibility > self.MIN_VISIBILITY and
            landmarks[self.RIGHT_ANKLE].visibility > self.MIN_VISIBILITY
        )

        left_knee_angle = self.calculate_angle(
            self.get_point(landmarks, self.LEFT_HIP),
            self.get_point(landmarks, self.LEFT_KNEE),
            self.get_point(landmarks, self.LEFT_ANKLE)
        )

        right_knee_angle = self.calculate_angle(
            self.get_point(landmarks, self.RIGHT_HIP),
            self.get_point(landmarks, self.RIGHT_KNEE),
            self.get_point(landmarks, self.RIGHT_ANKLE)
        )

        avg_knee_angle = (left_knee_angle + right_knee_angle) / 2

        if key_landmarks_visible:
            if avg_knee_angle > self.UP_THRESHOLD:
                self.stage = "up"

            if avg_knee_angle < self.DOWN_THRESHOLD and self.stage == "up":
                self.stage = "down"
                self.reps += 1

        # Form check: knees caving in (valgus check via knee x relative to ankle/hip x)
        left_knee_x = landmarks[self.LEFT_KNEE].x
        left_ankle_x = landmarks[self.LEFT_ANKLE].x
        
        if left_knee_x < left_ankle_x - self.KNEE_VALGUS_TOLERANCE:
            form_status = "KNEES CAVING IN"
        else:
            form_status = "GOOD FORM"

        return {
            "reps": self.reps,
            "knee_angle": int(avg_knee_angle),
            "form_status": form_status,
            "stage": self.stage
        }

    def _safe_angle(self, dx, dy):
        return math.degrees(math.atan2(abs(dx), abs(dy))) if dy != 0 else 0.0